...
public class Vector2D extends Point2D {
	...
	public static Vector2D add(Vector2D vec1, Vector2D vec2) {
		return new Vector2D(vec1.getX() + vec2.getX(), vec1.getY() +
														vec2.getY());
	}
	...
	// @Doc
	// Returns the direction that the vector is facing in radians
	// counterclockwise of the x axis. Uses Math.atan2(). Note that positive y's
	// point down.
	...
	public static Vector2D moveToPoint(Vector2D pos, Vector2D target,
			double speed) {
	...
	public static Vector2D createVecDirMag(double direction, double magnitude) {
		return new Vector2D(Math.cos(direction) * magnitude, Math
				.sin(direction) *
																magnitude);
	};

	public static Vector2D createRandom(Random r, int minX, int maxX, int minY,
			int maxY) {
		return new Vector2D(r.nextInt(maxX - minX) + minX, r.nextInt(maxY -
																		minY) +
															minY);
	}
