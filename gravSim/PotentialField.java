...
public class PotentialField {
	...
	PotentialField(Double gravityConstant, int fieldWidth, int fieldHeight,
		...
		for (int x = 0; x < fieldWidth_; x++) {
			...
			for (int y = 0; y < fieldHeight_; y++) {
				field_[x][y] =
						new PotentialFieldComponent(new Vector2D(
								x * distFieldPts + leftX_, y * distFieldPts +
															topY_));
			}
		}
		img_ =
	...
	public void addObjectToField(Vector2D objectPos, double mass,
		...
		for (int x = 0; x < fieldWidth_; x++) {
			for (int y = 0; y < fieldHeight_; y++) {
				field_[x][y].addToField(objectPos, distFieldPts_, mass, radius,
										gravityConstant_, usingCharge);
			}
		}
	}
...
class PotentialFieldComponent {
	...
	public void drawPixel(Graphics g, WritableRaster raster, int x, int y,
			int distFieldPts) {
	...
	public void addToField(Vector2D otherObject, double width,
			double otherObjectMass, double radius, double gravityConstant_,
			boolean usingCharge) {
		Vector2D vecTo =
				Vector2D.sub(	otherObject, Vector2D
										.sub(	pos_, new Vector2D(-width / 2,
														-width / 2)));
